...
void convLRFDataClass::create2dMap(){
    ...
    smd.cp.y=0;
    smd.cp.z=0;
    smd.index.resize(smd.heightInt*smd.widthInt);
    smd.pt.resize(hMax*wMax);
    //--インデックスデータ(sensorMapData)
    //インデックス初期化
    for(int h=0;h<smd.heightInt;h++){
        for(int w=0;w<smd.widthInt;w++){
            smd.index[h*smd.heightInt+w]=-1;
        }
    }
    //
    int k=0;//データカウンタ
    ...
    for(int i=0;i<dataSize;i++){
            ...
            std::isnan(d)) // 無限遠の場合
	    {
            ...
            if(convertToGrid(x_temp,z_temp,xi,zi)){//データ変換
                //インデックスデータ
                smd.index[zi*smd.widthInt+xi] = k;
                //マップデータ格納
